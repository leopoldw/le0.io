...
[Recompose](https://github.com/acdlite/recompose) is awesome. I've been using it on some larger React projects to supercharge functional components with sharable enhancers, some of which use features like state (yes, inside a functional component).
...
For reasons then unknown, Recompose development has stagnated for a while. This was disappointing as the APIs it provided to create sharable, composable enhancers felt essential in a way no other React library had before. Then, shortly after the announcement of [React Hooks](https://reactjs.org/docs/Hooks-intro.html), Recompose announced that it would cease active new feature development. Things began to make sense. As per a note from core developer [Andrew Clark](https://twitter.com/acdlite):
...
I'm glad you asked - but feel free to skip this chapter if you already know the basic rundown. Recompose is an enhancer library, providing convenient [Higher Order Components](https://reactjs.org/docs/higher-order-components.html) (HOCs) for common React patterns, allowing us to incrementally add features and functionality to a component via [composition](https://www.robinwieruch.de/react-component-composition/). This provides some significant benefits:
...
Write a _logicless_ functional (or UI) component. This component should avoid contaning logic statements, instead implicitly returning JSX with the assumption that the it will magically recieve all the props it needs with no additional logic.
...
As each enhancer is a complete component, React Devtools could easily become a complete mess. A single inner component wrapped in numerous enhancers would create an unwieldly, massive component tree - displayed as something resembling [callback hell](http://callbackhell.com/). This made debugging in Devtools painful and difficult.

<LazyImage fileName="devtoolsrecompose.png" caption="Check out the catchy component names too!" />

Internally, React does not delineate between components that render their own JSX vs components that simply pass something down to an inner component. What we need is a new way to enhance our components without creating additional intermediete components in the process.
...
They don't contain any `render` functionality, and they don't need to. Instead, they simply supply React components with _something_ they need & handle rerendering the parent component when required. As they are simply functions, the underlying component tree is significantly decluttered.
...
While `withState` is just simple getter/settler that will completely replace the entire state each time, `withStateHanders` allows us to capture more complex logic.
...
Since Hooks don't explicity plug into component props like this, we can instead use a vanilla JavaScript function to encapsulate the logic found within `withProps`.
...
Handlers have historically been a bit tricky to deal with in React, since they often need to access both the arguments passed to them _and_ the current props of the component. We can `bind` the props we need to the handler, or create an anonmous inline function which calls the handler with the props in the parent scope.
...
This enahncer can render one of two components depending on the props passed from the chain.
...
The enhancer pattern is incredibley useful and deserves to be part of the [official top-level API](https://reactjs.org/docs/react-api.html#hooks), as it now is with Hooks. Even though they're still fresh, I'm sold on Hooks as a Recompose replacement thanks to their simplicity, flexibility and ease of use.