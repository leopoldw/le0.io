...
I'm glad you asked. When working on a React project with complex and ever-changing requirements, it's easy for that complexity to reflect in the codebase as a tangle of bloated logic which, over time, becomes harder and harder to test and reason about.

// REMOVETHIS
An initial approach may look something like this:

- Create a simple functional component
- Requirements change, and some kind of state or lifecycle hook becomes necessary
- Begin the process of converting our straightforward functional component to a class-based component (cue PTSD)
- Over time, add a variety of class methods which perform some function to aid in rendering. Since these functions can be placed and used practically anywhere in the class, reasoning about component logic becomes a matter of jumping around the file, chasing strings of logic.

There are a [myriad](https://medium.com/@Zwenza/functional-vs-class-components-in-react-231e3fbd7108) [of](https://frontarm.com/james-k-nelson/es6-classes-vs-function-components/) [articles](https://www.andreasreiterer.at/react-functional-components/) in the wild addressing the class/functional component debate, but ultimatley my main concerns were:

// pros/cons

- Breaking up logical complexity into smaller chunks which are easier to reason about
- Removing dependencies between logical chunks so they become agnostic about where and how they're used
- DRYing up code by sharing related logic chunks where helpful

// TODO: logicless functional

# API Replacements

Finally - let's see some code!

// TODO: not all are hooks

<Aside>React docs highly recommend starting hook names with "use", ef "useGreeting".</Aside>

// TODO: hooks are just functions
...
const useGoodbye = name => `Hello, ${name}!`
```
